...
Veysseire Daniel & Fabre Mickael \\
\end{tabular}
 \\
     \\
...
Nous interprèterons ensuite les résultats obtenus puis nous finirons par une conclusion sur l'efficacité des différentes méthodes et paramétrisations.
...
\section{Introduction}
	   ...
	   Ci-dessous un schéma illustrant les différentes étapes de la chaîne de reconnaissance:
...
Nous allons comparer différentes paramétrisations de données et ne conserver que celles qui offrent les meilleurs résultats.


\vspace{1\baselineskip}
\vspace{1\baselineskip}

\textbf{Transformée de Fourrier Rapide (FFT)}
...
Chaque élément placé ligne i et colonne j dans la matrice vaut cov(Xi, Xj) avec Xi la ieme composante du vecteur. Ainsi sur la diagonale se situent les variances de chaque composante du vecteur. La covariance se calcule à l'aide de la formule suivante:
\begin{center}
...
En utilisant la densité de probabilité de la loi normale multidimenssionelle on obtient:
...
Nous allons utiliser une méthodologie simple et naturelle pour comparer nos résultats. Du fait du manque de données (nous n'avons qu'un échantillon de 100 signaux pour chacune des 10 classe) nous utiliserons la validation croisée et rappelerons brièvement son principe.

\subsection{principe de la validation croisée}

Nous allons réaliser une validation croisée. C'est à dire que nous allons faire varier nos échantillons d'apprentissage et nos échantillons de test et faire une moyenne avec les résultats trouvés. Notre premier échantillon d'apprentissage est composé des 80 premiers échantillons de chaque classe et les 20 restants pour les tests. On refait une validation mais cette fois ci en prenant les échantillons de 21 à 100 pour l'apprentissage et 1 à 20 pour les tests. Puis 41 à 100 et 1 à 20 pour l'apprentissage et 21 à 40 pour les test, et on continue ainsi en décalant de 20.
...
D'après le résultat de notre évaluation, la méthode de classification par loi normal multidimenssionelle offre de meilleurs résultats avec la transformée de Fourrier qu'avec la paramétrisation cepstral, alors que la méthode de classification des K plus proche voisin offre de meilleur résultat pour la paramétrisation cepstral. Cependant les résultats peuvent être faussé, on observe dans le cas de K Plus proche voisins que pour l'échantillon 2, la paramétrisation par transformée de Fourrier a produit un résultat anormalement décevant par rapport aux autres échantillons, ce qui a diminué de beaucoup son efiicacité finale, alors que pour le même échantillon, la paramétrisation cepstral a fournit de bons résultats. Dans la majorité des cas la parémétrisation FFT semble toutefois être plus efficace que la paramétrisation cepstral.
\section{conclusion}

 L'échantillon dont nous diposons n'est pas assez grand pour se prononcer clairement sur la meilleur méthode de classification. En revanche, on peut affirmer que la paramétrisation par MFCC est très efficace dans ces conditions d'évaluation; quelque soit la méthode de classification, elle n'a même souffert d'aucune erreur. 
 
 Nous nous sommes placé, dans le cas de cet article, dans des conditions idéales, tous les échantillons sonores font la même taille, et seuls des voyelles sont identifiées. Il pourait être intéressant de tester la parametrisation MFCC avec des conditions moins idéales pour tester ses limites. On peut aussi se tourner vers un mélange de loi gaussienne pour la méthode de classification afin d'obtenir de meilleurs résultats.

\subsection*{Annexe}

Programmes utilisés:
Matlab, voicebox, \LaTeX
\vspace{1\baselineskip}

\subsection*{Remerciements}
Merci à Jérôme Farinas pour ses explications et son aide,
et à Régine André-Obrecht pour ses cours sur les méthodes de décision et sur la classification automatique.
...
Veysseire Daniel : wedg@hotmail.fr

Fabre Mickael : mickaelfabre@free.fr
\end{document}
