...
public:
  ...
  //coeff sert a mettre les durees en valeurs entieres
  ...
  int* DFS()
  {
	...
	int i=0;
        ...
        do
        {
		//tant qu'il y a des successeurs
		while(i<activiteCourante.n_s){
		
			//si le successeur n'est pas visite
                    if(!visited[(activiteCourante.Succ.at(i).i]){
			
                                numVisite++;

                                activiteCourante=activiteCourante.Succ.at(i);
		...
		activiteCourante=stack.back(activiteCourante);


		stack.pop_back;

                visited[activiteCourante.i] = true;
		numVisAc(activiteCourante) = numAction;
