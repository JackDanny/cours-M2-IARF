...
public final class CollieModelPanel extends JPanel implements
	...
	protected CollieModelPanel() {
		...
		this.addPropertyChangeListener(MODEL_DIRTIED_CHANGED_PROPERTY,
	...
	public class AddNoteAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent event) {
			System.out.println("AddNoteAction.actionPerformed: "
	...
	public class RelationToolAction extends AbstractAction {
		...
		public RelationToolAction(String label, Icon icon, RelationToolIF tool,
				int cursor) {
			super(label, icon);
		...
		public void actionPerformed(ActionEvent event) {
			...
			CollieModelPanel.this.setCursor(Cursor
					.getPredefinedCursor(theCursor));
		}
	...
	public class ModelPanelMouseListener extends MouseAdapter {
		...
		public void mousePressed(MouseEvent event) {
			...
			CollieModelPanel.this.diagram.deselectAll();
			CollieModelPanel.this.selectedElement = CollieModelPanel.this.diagram
			...
			if (SwingUtilities.isRightMouseButton(event)) {
				if (CollieModelPanel.this.selectedElement != null)
					CollieModelPanel.this.selectedElement.doRightMousePressed(
							event, CollieModelPanel.this.graphicsContext);
				else {
					// panelPopup.show(CollieModelPanel.this, event.getX(),
					// event.getY());
				}
			} else /** if(SwingUtilities.isLeftMouseButton(event)) **/
			{
				if (CollieModelPanel.this.selectedElement != null) {
					...
					CollieModelPanel.this.theRelationTool.doLeftMousePressed(
		...
		//
		// If the mouse release is from the left button, then release the
		// element
		// that was dragged.
		//
		public void mouseReleased(MouseEvent event) {
			if (CollieModelPanel.this.selectedElement != null
				...
				CollieModelPanel.this.theRelationTool.doLeftMouseReleased(
						event, selectedElement);
			}
	...
	public class ModelPanelMouseMotionListener extends MouseMotionAdapter {
		...
		public void mouseDragged(MouseEvent event) {
			if (CollieModelPanel.this.selectedElement != null
					&& SwingUtilities.isLeftMouseButton(event))
				CollieModelPanel.this.theRelationTool.doLeftMouseDragged(event,
						selectedElement);
		}
