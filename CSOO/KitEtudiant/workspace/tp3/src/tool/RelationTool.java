...
public abstract class RelationTool implements RelationToolIF {
	...
	/**
	 * Use the relation tool prepare for possible dragging and creation of a
	 * relation model element. Dragging can result in either one or two line
	 * segments according to the relative positions of the two node boxes. If
	 * there is only one then we refer to it as the second segment. If there are
	 * two segments then the second is nearest the mouse.
	 */
	public void doLeftMousePressed(MouseEvent event,
		...
		if (selectedElement != null) {
			...
			theSecondSegment = new LineSegment(new Point(event.getPoint()),
	 ...
	 * destination for the relation. Prepare a new relation, add it to the model
	 * and advise both nodes of its inclusion. The final segment is clipped to
	 * the destination node edge before establishing the actual relation.
	 */
	public void doLeftMouseReleased(MouseEvent event,
			ModelElement selectedElement) {
		CollieModelPanel modelPanel = CollieModelPanel.getCollieModelPanel();
		...
		if (destinationSymbol != null) {
			...
			if (theSourceNode != theDestinationNode) {
				...
				final Rectangle destinationBounds = theDestinationNode
						.getBounds();

				switch (theDestinationNode.getCompassBearing(theSecondSegment
						.getStart())) {
				...
				case NodeElement.EAST:
					theSecondSegment.setFinish(destinationBounds.x
							+ destinationBounds.width, finish.y);
					break;
				...
				LinkageElement relation = this.makeLinkage(theSourceNode,
						theDestinationNode, theFirstSegment, theSecondSegment);
	...
	/**
	 * At its simplest we simply obliterate the old segment(s) and draw anew the
	 * new segment(s). According to the mouse movements and its geographic
	 * position relative to the source node we may create or remove the first
	 * segment. The segment adjacent to the source node is clipped to the
	 * appropriate geographic boundary.
	 */
	public void doLeftMouseDragged(MouseEvent event,
			ModelElement selectedElement) {
		if (selectedElement != null) {
			...
			if (theSourceNode.isHit(event.getPoint()) == false) {
				...
				case NodeElement.SOUTHEAST:
					if (theFirstSegment != null) {
						...
						case NodeElement.NORTHEAST:
						case NodeElement.SOUTHEAST:
							theFirstSegment.setStart(bounds.x + bounds.width,
									firstStart.y);
							break;
						}

						theFirstSegment.setFinish(eventPoint.x,
								theFirstSegment.getFinish().y);
						...
						theFirstSegment = new LineSegment(new Point(startX,
								startY), new Point(eventPoint.x, eventY));
					}
					break;
	...
	public void doRightMousePressed(MouseEvent event,
			ModelElement selectedElement) {
