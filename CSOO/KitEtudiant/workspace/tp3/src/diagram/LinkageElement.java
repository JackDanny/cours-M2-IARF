...
public class LinkageElement extends ModelElement {
	...
	public LinkageElement(NodeElement source, NodeElement destination,
	...
	public void doLeftMousePressed(MouseEvent event, Graphics graphicsContext) {
		...
		for (int k = 0; k < size; k++) {
			...
			if (theSelectedSegment != null
					&& theSelectedSegment.isHit(event.getPoint())) {
				theSelectedIndex = k;
				break;
			}
		}
	...
	public void doLeftMouseDragged(MouseEvent event, Graphics graphicsContext) {
		if (theSelectedSegment != null) {
			...
			final boolean selectedSegmentIsHorizontal = theSelectedSegment
					.isHorizontal();
			...
			if (selectedSegmentIsHorizontal) {
				theSelectedSegment.setStart(theSelectedSegment.getStart().x,
						mousePoint.y);
				theSelectedSegment.setFinish(theSelectedSegment.getFinish().x,
						mousePoint.y);
			} else {
				theSelectedSegment.setStart(mousePoint.x,
						theSelectedSegment.getStart().y);
				theSelectedSegment.setFinish(mousePoint.x,
						theSelectedSegment.getFinish().y);
			}

			if (theSelectedIndex == 0)
				this.processFirstSegment(selectedSegmentIsHorizontal,
						mousePoint);
			if (theSelectedIndex == theLineSegments.size() - 1)
				this.processFinalSegment(selectedSegmentIsHorizontal,
						mousePoint);
			if (theSelectedIndex > 0)
	...
	private void processFirstSegment(boolean segmentIsHorizontal,
			Point mousePoint) {
		final Rectangle sourceBounds = theSourceNode.getBounds();
		if (segmentIsHorizontal && !theSourceNode.isLatitudinal(mousePoint)) { // insert
																				// new
																				// vertical
																				// segment
			theSelectedSegment.setStart(
					(sourceBounds.x + sourceBounds.x + sourceBounds.width) / 2,
					theSelectedSegment.getStart().y);

			final int startX = theSelectedSegment.getStart().x;
			final int startY = (theSourceNode.isNortherly(mousePoint)) ? sourceBounds.y
					: sourceBounds.y + sourceBounds.height;

			LineSegment newSegment = new LineSegment(startX, startY,
					theSelectedSegment.getStart().x,
					theSelectedSegment.getStart().y);
			this.insertSegment(0, newSegment);
			theSelectedIndex++;

		} else if (!segmentIsHorizontal
				&& !theSourceNode.isLongitudinal(mousePoint)) { // insert new
																// horizontal
																// segment
			theSelectedSegment
					.setStart(theSelectedSegment.getStart().x, (sourceBounds.y
							+ sourceBounds.y + sourceBounds.height) / 2);

			final int startX = (theSourceNode.isWesterly(mousePoint)) ? sourceBounds.x
	...
	private void processFinalSegment(boolean segmentIsHorizontal,
			Point mousePoint) {
		final Rectangle destinationBounds = theDestinationNode.getBounds();
		if (segmentIsHorizontal
				&& !theDestinationNode.isLatitudinal(mousePoint)) { // insert
																	// new
																	// vertical
																	// segment
			theSelectedSegment.setFinish((destinationBounds.x
					+ destinationBounds.x + destinationBounds.width) / 2,
					theSelectedSegment.getFinish().y);

			final int finishX = theSelectedSegment.getFinish().x;
			final int finishY = (theDestinationNode.isNortherly(mousePoint)) ? destinationBounds.y
					: destinationBounds.y + destinationBounds.height;

			LineSegment newSegment = new LineSegment(
					theSelectedSegment.getFinish().x,
					theSelectedSegment.getFinish().y, finishX, finishY);
			this.addSegment(newSegment);

		} else if (!segmentIsHorizontal
				&& !theDestinationNode.isLongitudinal(mousePoint)) { // insert
																		// new
																		// horizontal
																		// segment
			theSelectedSegment
					.setFinish(
							theSelectedSegment.getFinish().x,
							(destinationBounds.y + destinationBounds.y + destinationBounds.height) / 2);

			final int finishX = (theDestinationNode.isWesterly(mousePoint)) ? destinationBounds.x
	...
	private void processSecondSegment() {
		final Rectangle sourceBounds = theSourceNode.getBounds();
		final LineSegment precedingSegment = (LineSegment) theLineSegments
				.get(theSelectedIndex - 1);
		precedingSegment.setFinish(theSelectedSegment.getStart().x,
				theSelectedSegment.getStart().y);
		...
		if (theSelectedIndex == 1) {
			if (sourceBounds.contains(precedingSegment.getFinish())) {
				...
				final int bearing = theSourceNode
						.getCompassBearing(theSelectedSegment.getFinish());
				switch (bearing) {
				case NodeElement.NORTH:
					theSelectedSegment.setStart(
							theSelectedSegment.getStart().x, sourceBounds.y);
					break;
				...
				case NodeElement.EAST:
					theSelectedSegment.setStart(sourceBounds.x
							+ sourceBounds.width,
							theSelectedSegment.getStart().y);
					break;
	...
	private void processPenultimateSegment() {
		final Rectangle destinationBounds = theDestinationNode.getBounds();
		final LineSegment followingSegment = (LineSegment) theLineSegments
				.get(theSelectedIndex + 1);
		followingSegment.setStart(theSelectedSegment.getFinish().x,
				theSelectedSegment.getFinish().y);
		...
		if (theSelectedIndex == theLineSegments.size() - 2) {
			if (destinationBounds.contains(followingSegment.getStart())) {
				...
				final int bearing = theDestinationNode
						.getCompassBearing(theSelectedSegment.getStart());
				switch (bearing) {
				case NodeElement.NORTH:
					theSelectedSegment.setFinish(
							theSelectedSegment.getFinish().x,
							destinationBounds.y);
					break;
				case NodeElement.SOUTH:
	...
	/**
	 * The given node has moved and this link must be updated to reflect the new
	...
	public void update(NodeElement movedNode, int offsetX, int offsetY) {
		if (theSourceNode == movedNode) {
			...
			if (theLineSegments.size() == 1) {
				...
				this.updateCrossOver(theDestinationNode, segment.getFinish(),
						segment.getStart());
			}
		}
		...
		if (theDestinationNode == movedNode) {
			...
			if (theLineSegments.size() == 1) {
				LineSegment segment = (LineSegment) theLineSegments.get(0);
				this.updateCrossOver(theSourceNode, segment.getStart(),
						segment.getFinish());
			}
		}
	}
	...
	/**
	 * The source node has moved. There are two situations to consider: the node
	 * has been positioned on an existing segment or it has moved away from the
	 * first segment.
	 */
	private void updateMovedSourceNode(NodeElement movedNode) {
		//
		// Test to see if the node has relocated itself on to an existing
		// segment.
		...
		for (int k = size - 1; k >= 0; k--) {
			...
			if (start.x < finish.x || start.y < finish.y)
				segmentRectangle = new Rectangle(start.x - 1, start.y - 1,
						finish.x + 2 - start.x, finish.y + 2 - start.y);
			else
				segmentRectangle = new Rectangle(finish.x - 1, finish.y - 1,
						start.x + 2 - finish.x, start.y + 2 - finish.y);

			if (bounds.intersects(segmentRectangle)) {
				for (int j = k - 1; j >= 0; j--)
					this.removeSegment(j);

				final int bearing = movedNode.getCompassBearing(segment
						.getFinish());
				switch (bearing) {
				case NodeElement.NORTH:
				...
				case NodeElement.SOUTH:
					...
					break;
				case NodeElement.EAST:
					segment.setStart(bounds.x + bounds.width,
							segment.getFinish().y);
					break;
				case NodeElement.WEST:
		...
		//
		// Otherwise, the source has moved away from the connecting segment
		// S[0]. Either
		...
		if (firstSegment.isHorizontal())
			additionalSegment = this.updateSourceHorizontalSegment(movedNode,
					firstSegment);
		else
			additionalSegment = this.updateSourceVerticalSegment(movedNode,
					firstSegment);
	 ...
	 * is horizontal.
	 */
	private LineSegment updateSourceHorizontalSegment(NodeElement movedNode,
		...
		final int bearing = movedNode
				.getCompassBearing(firstSegment.getStart());
		switch (bearing) {
		case NodeElement.NORTHWEST:
			firstSegment.setStart((bounds.x + bounds.x + bounds.width) / 2,
					firstY);
			additionalSegment = new LineSegment(firstSegment.getStart().x,
					bounds.y, firstSegment.getStart().x,
					firstSegment.getStart().y);
			break;
		case NodeElement.NORTH:
			additionalSegment = new LineSegment(firstX, bounds.y,
					firstSegment.getStart().x, firstSegment.getStart().y);
			break;
		...
		case NodeElement.SOUTHWEST:
			firstSegment.setStart((bounds.x + bounds.x + bounds.width) / 2,
					firstY);
			additionalSegment = new LineSegment(firstSegment.getStart().x,
					bounds.y + bounds.height, firstSegment.getStart().x,
					firstSegment.getStart().y);
			break;
		case NodeElement.SOUTH:
			additionalSegment = new LineSegment(firstX, bounds.y
					+ bounds.height, firstSegment.getStart().x,
					firstSegment.getStart().y);
			break;
	 ...
	 * is vertical.
	 */
	private LineSegment updateSourceVerticalSegment(NodeElement movedNode,
			LineSegment firstSegment) {
		final Rectangle bounds = movedNode.getBounds();
		LineSegment additionalSegment = null;
		...
		switch (bearing) {
		case NodeElement.NORTHWEST:
			firstSegment.setStart(firstX,
					(bounds.y + bounds.y + bounds.height) / 2);
			additionalSegment = new LineSegment(bounds.x,
					firstSegment.getStart().y, firstSegment.getStart().x,
					firstSegment.getStart().y);
			break;
		case NodeElement.NORTH:
		...
		case NodeElement.NORTHEAST:
			firstSegment.setStart(firstX,
					(bounds.y + bounds.y + bounds.height) / 2);
			additionalSegment = new LineSegment(bounds.x + bounds.width,
					firstSegment.getStart().y, firstSegment.getStart().x,
					firstSegment.getStart().y);
			break;
		case NodeElement.WEST:
			additionalSegment = new LineSegment(bounds.x, firstY,
					firstSegment.getStart().x, firstSegment.getStart().y);
			break;
		case NodeElement.CENTRE:
			break;
		case NodeElement.EAST:
			additionalSegment = new LineSegment(bounds.x + bounds.width,
					firstY, firstSegment.getStart().x,
					firstSegment.getStart().y);
			break;
		case NodeElement.SOUTHWEST:
			firstSegment.setStart(firstX,
	 ...
	 * node has been positioned on an existing segment or it has moved away from
	 * the final segment.
	 */
	private void updateMovedDestinationNode(NodeElement movedNode) {
		...
		for (int k = 0; k < size; k++) {
			...
			if (bounds.intersects(segmentRectangle)) {
				...
				case NodeElement.SOUTH:
					...
					break;
				case NodeElement.EAST:
					segment.setFinish(bounds.x + bounds.width,
							segment.getStart().y);
					break;
				case NodeElement.WEST:
		...
		//
		// Otherwise, the node has moved away from the connecting segment
		// S[size-1]. Either
		// we extend the final segment, or we both extend it and add another
		// new final segment.
		//
		LineSegment additionalSegment = null;
		LineSegment finalSegment = (LineSegment) theLineSegments
				.get(theLineSegments.size() - 1);

		if (finalSegment.isHorizontal())
			additionalSegment = this.updateDestinationHorizontalSegment(
					movedNode, finalSegment);
		else
			additionalSegment = this.updateDestinationVerticalSegment(
					movedNode, finalSegment);
	...
	/**
	 * The destination node has moved away from the final segment. This final
	 * segment is horizontal.
	 */
	private LineSegment updateDestinationHorizontalSegment(
			NodeElement movedNode, LineSegment finalSegment) {
		LineSegment additionalSegment = null;
		...
		final int bearing = movedNode.getCompassBearing(finalSegment
				.getFinish());
		switch (bearing) {
		case NodeElement.NORTHWEST:
	 ...
	 * segment is vertical.
	 */
	private LineSegment updateDestinationVerticalSegment(NodeElement movedNode,
	...
	protected ArrayList<LineSegment> theLineSegments = new ArrayList<LineSegment>(8);
